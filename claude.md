...
| `engineering.js` | Warp core trip, emergency battery, ablative armour processing, shield regen rate calculation, repair queue (2 independent teams), engineering matrix UI, power allocation (`tuneBusAllocation`), power presets (`applyPowerPreset`), EPS conduit conduction + thermal buildup, system degradation thresholds, shield manipulation. **Engineering combat toolkit** (active combat agency for the engineer): `divertPowerToWeapons` (⚡ Weapons Power Surge — +35% weapon dmg + ×1.8 weapon-cap recharge, shields −50%, 8s/28s CD), `toggleFireCoordination` (🎯 crew fires aggressively/torpedo-heavy), `engageEmergencyForcefields` (🛡 incoming −45%, 5s/30s CD); `tickEngineeringToolkit` (timers, called from `processNewMechanicsTimers`), `_updateEngToolkitButtons` (live button state). Effects route through `fireSelectedArray` dmg, `computeConduitConduction` cap, and the enemy-fire paths so they help the auto-crew too |
...
| `auto-delegation.js` | Computer management of uncrewed stations (`processAutomatedDelegation`): auto-engineering relay resets + repair dispatch, auto-tactical fire cycle, captain-mode Worf/O'Brien/Nog autonomous behaviours. **Smart auto-tactical** (`_autoTacticalFire`/`_autoRotateFire`): vs Borg, targets the Regenerative Matrix then rotates lowest-resistance weapons + spends overload/burst tools (solves the adaptation puzzle); conventional fights use salvo + burst + torps. **Auto-O'Brien** wields the engineering toolkit (surge/forcefields/fire-coordination) so Helm/Captain players get the same support — this is what makes the Borg + wolfpack winnable from the non-tactical stations |
...
| `smoketest.js` | Invariant + behaviour test harness (`runSmokeTests`). Drives every ship × station × enemy combo through the real per-frame ticks (no RAF/canvas), exercises every special ability, and runs scenario tests (repair heals, warp-core trip scales power, emergency warp / ramming end the game, wolfpack promotion+victory). Asserts no exception + finite/in-range values + firing deals damage. **Deterministic sim clock (`_simClockInstall`/`_simClockAdvance`/`_simClockRestore`):** shims `setTimeout`/`performance.now` to sim-time so delayed gameplay effects (cloak transition, decloak alpha strike, Borg tractor 8s release, burst/torpedo staggers) actually fire in the synchronous loop instead of being silently skipped — installed per engagement by `_smokeStartEngagement`, drained each `_smokeAdvance`, and **auto-restored on the next real event-loop turn** (via the saved native `setTimeout`) so it can never leak into live play. Without it, `setTimeout`-driven mechanics never executed and playtests were wrong (e.g. the tractor stayed ON forever). Auto-runs on `?test=1` (publishes `window.__SMOKE_RESULT` / `__SMOKE_DONE`); or call `runSmokeTests()`. Runs in CI via `tests/run-smoke.mjs` (Playwright headless Chromium) on every push/PR — see `.github/workflows/smoke.yml` |
...
> **Per-level campaign escalation (`scale` in `CAMPAIGN_ORDER`):** each level carries a campaign-only `scale:{hull,dmg,lock,bypass}` applied on top of its base `diff` by `_applyCampaignEscalation()` (campaign.js), so each level is a genuine step up. `hull` lengthens the fight; `lock` makes torpedo bursts more frequent (those breach shields); `dmg` raises hit size (runtime mult `G.campaignDmgMult`); **`bypass`** routes a fraction of every shot straight to hull ignoring shields/regen (runtime `G.campaignBypass`) — the only lever that reliably costs a well-defended player hull. **The scale is applied in BOTH modes** so a given enemy fights at identical difficulty whether in a single engagement or a campaign level: campaign applies it by level via `_launchCampaignLevel`; single engagements apply it by archetype lookup in `initiateVesselSimulation` (the `if (!G.campaignMode) _applyCampaignEscalation()` call — guarded so campaign doesn't double-apply). Mults reset to 1.0/0 in `enemyResetForBattle`, then re-applied. The remaining cross-mode difference is only the player-chosen difficulty tier (single) vs the fixed per-level `diff` (campaign). **Bypass interacts multiplicatively with each enemy's burst profile, so intrinsically punchy ships need LESS scaling than their hull suggests** — Vor'Cha (close-range ×1.35 disruptors + heavy torps) is hypersensitive (bypass 0.075), the slow-firing cloaky Warbird needs far more (0.19). Values are tuned empirically, not by raw stat line.
>
> **Measured curve (scripted Defiant/tactical, 12 trials/level — avg hull lost · win rate):** L1 0%·12/12 · L2 3%·12/12 · L3 4%·12/12 · L4 14%·12/12 · L5 25%·11/12 · L6 29%·12/12 · L7 64%·4/12 · L8 87%·3/12 · L9 90%·0/12. Monotonic in hull-lost and win-rate; the L6→L7 jump is the deliberate "Dominion war begins" gate into the endgame. Re-run via the scripted harness (see git history / a playthrough loop calling `_smokeStartEngagement` + `_applyCampaignEscalation`) after any enemy/weapon change.
>
> **Winnability verified — every level is beatable with optimal play** (the curve above uses a *fixed-pattern* player, which is why win-rate drops at the top; with the right per-enemy strategy each level wins): L1–L6 reliably (7–8/8), L7 5/8, L8 (pack) 3/8 (the hardest *conventional* fight — a 3-ship bypass swarm). **The Borg is a strategy-gated puzzle boss:** a fixed-pattern player goes ~0/12, but the correct play — **target the Regenerative Matrix (`shields_sys`) first to gut shield regen, then burst the hull with overcharge/max-pulse while rotating weapon types** — wins **6/6 decisively (~60s, ~98% hull)**. (Harness caveat: the Borg tractor clears via `setTimeout(8000)` which doesn't fire in a synchronous sim — emulate the 8s release in sim-time or the player's weapons stay frozen.)
...
128. **After a campaign ended, "Return to Bridge" gave an empty setup wizard — no way to start another battle.** `concludeCampaignLevel` hides four *inner* setup sections (`ship-select-section`, `setup-controls-anchor`, `campaign-diff-section`, `campaign-run-section`) to declutter the campaign-summary modal, but `_setupReset()` only restored the wizard *step* wrappers (`setup-step-mode/configure/single/campaign`), leaving those inner sections `display:none`. Walking the wizard again reached the configure step with an invisible ship picker + station buttons (and the single step with no difficulty UI), so the player couldn't engage. Fixed by restoring all four inner sections to `display:''` in `_setupReset()` (the canonical wizard-reset, called by `returnToSetup`). Verified end-to-end: finish campaign → Return to Bridge → wizard shows ship/station/difficulty/engage and a new battle starts.
129. **Widespread mojibake in index.html UI text** (e.g. `âš¡ BATTLE STATIONS â€"`, `â—† ELITE`) — root cause was `cachebust.ps1` reading `index.html` with `Get-Content -Raw` and no `-Encoding`, so PowerShell 5.1 decoded UTF-8 emoji/em-dash/arrow bytes as Windows-1252 and the subsequent UTF-8 `Set-Content` baked the corruption in on every run (compounding each run). Added `-Encoding utf8` to the read and repaired the corrupted bytes. Now renders ▶ ⚡ — → ≤ × ● ◎ ◆ correctly.
130. **Captain's Chair showed "Engage Cloak" for the Enterprise-E** (which has saucer separation, not a cloak). `capCloakToggle` already routed to `toggleSaucerSeparation`, but `_updateCloakButtonLabel` only ever wrote cloak labels. Made it ship-aware — Enterprise shows Saucer Separation / Reconnect Saucer / Reconnecting Ns / Saucer CD Ns — and the toggle crew line is state-aware.

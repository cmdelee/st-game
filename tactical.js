...
function updateHelmPanel() {
  const _active  = 'background:#ffffff;color:#000;box-shadow:0 0 10px rgba(255,255,255,0.5);';
  const _inactive = 'background:;color:;box-shadow:;';
  ...
  ['stop','maneuvering','half','full'].forEach(s => {
    ...
    if (G.helmSpeed === s) {
      btn.style.cssText = btn.style.cssText.replace(/background:[^;]*;?/g,'').replace(/color:[^;]*;?/g,'').replace(/box-shadow:[^;]*;?/g,'');
  ...
  // Vector buttons — live shield HP + white glow on active
  ...
  // Range buttons — white glow on active
